...
- `ml/` — Python. Data pipeline, feature engineering, and training for the **v4 XGBoost classifier** (single tree ensemble). Produces `xgboost.onnx` + `xgboost.joblib` in `ml/models/`. Not imported by the app at runtime — only the exported `.onnx` is. (Earlier iteration stacked XGB + CatBoost through a meta-LR; dropped because stacking added +0.001 AUC on two tree models.)
...
1. **ML filer-risk layer** (`lib/classifier.ts`) — v4 XGBoost ONNX scoring the *filer*, not the mark. Inputs: 13 `FEATURE_COLS` built by `lib/features.ts`. Only a `score > 0.85` short-circuits to `high_risk` with `source: 'ml_only'`. There is no low short-circuit — every non-high request runs the agent.
...
- Mapped by `applicantToFeatures()` into **8 of the 13 ML feature columns** (the "applicant-side" features). The remaining 5 are mark-side features computed by `markFeaturesFromPriorArt()` via a normalized-name lookup in SQLite, falling back to `UNKNOWN_MARK_FEATURES` priors when no prior art exists.
...
`loadClassifier()` resolves `DEFAULT_MODELS_DIR = ../ml/models` relative to `process.cwd()`. This only works when the Next dev server is started from `app/`. The loader expects `xgboost.onnx`. If it's missing, the classifier throws `model_load` at first call, not at startup.
